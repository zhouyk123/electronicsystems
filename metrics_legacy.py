...
LEFT_TURN_PID = {"P":30, "I":0.06, "D":20}
RIGHT_TURN_PID = {"P":40, "I":0.01, "D":23}
...
def set_turn_pid_mode(mode, duty=0):
    ...
    with turnPidLock:
        ...
        if mode == 'left':
            turnPidController = WheelSpeedPID(**RIGHT_TURN_PID, target=target_speed)
        elif mode == 'right':
            turnPidController = WheelSpeedPID(**LEFT_TURN_PID, target=target_speed)
        turnPidMode = mode
...
if __name__ == '__main__':
    ...
    pidController = WheelSpeedPID(P=P, I=I, D=D, target=0, lb=-8, ub=8)
    ...
    if mode == 'red':
        ...
        if red_position == "left":
            ...
            # Step3-1: 左转避开魔方 (L295)
            print(f"[RED-第3步] left路径: 左转 duty={TURN_DUTY} time={TURN_LEFT_TIME}s")
            turn_left(TURN_DUTY)
            time.sleep(TURN_LEFT_TIME)
            stop()
            time.sleep(INTERVAL_SLEEP_TIME)
            # Step3-1: 直行绕到魔方侧面 (L297)
            print(f"[RED-第3步] left路径: 直行 duty={TURN_DUTY} time={STRAIGHT_TIME}s")
            go_straight(TURN_DUTY)
            time.sleep(STRAIGHT_TIME)
            stop()
            time.sleep(INTERVAL_SLEEP_TIME)
            # Step3-1: 右转回正车头 (L301)
            print(f"[RED-第3步] left路径: 右转回正 duty={TURN_DUTY} time={TURN_RIGHT_TIME}s")
            turn_right(TURN_DUTY)
            ...
            print(f"[RED-第3步] left路径: 直行通过魔方 duty={TURN_DUTY_FAST} ratio={RATIO} time={PASS_STRAIGHT_TIME}s")
            go_straight(STRAIGHT_DUTY, RATIO)
